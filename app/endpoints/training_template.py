...
def read_training_templates(db: Session = Depends(get_db)):
...
def read_training_template(template_id: int, db: Session = Depends(get_db)):
...
def create_training_template_endpoint(
        training_template: TrainingTemplateCreate, db: Session = Depends(get_db)
):
...
def update_training_template_endpoint(
        template_id: int, training_template: TrainingTemplateUpdate, db: Session = Depends(get_db)
):
...
def delete_training_template_endpoint(template_id: int, db: Session = Depends(get_db)):
